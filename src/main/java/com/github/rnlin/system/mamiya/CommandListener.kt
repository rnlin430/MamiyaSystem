...
class CommandListener(private val plugin: MamiyaSystemPlugin, private val we: WorldEditPlugin) : CommandExecutor {
    ...
    override fun onCommand(sender: CommandSender, command: Command, label: String, args: Array<String>): Boolean {
        ...
        if (command.label.equals(MamiyaSystemPlugin.COMMANDS[0], ignoreCase = true)) {
            if (args.size == 0) {
                if (!inspection(sender, command, args)) return true
